...
## v1.0.4-test8 - 2026-09-07

- 新增默认开启的 `IgnoreBuildZones`：跳过独立及附属于家具的禁建区域，并继续射线；接收端也不再将其转换为家具目标。关闭时恢复旧的附属区域处理。
- 保留准星精确实体优先、球扫兜底、实体遮挡和工作台本体选择；过滤重试仍有上限，不扫描全场景。
- 文字投影保持 20 Hz；物体边界衍生的相对偏移按 4 Hz 刷新，位置仍实时跟随。形状或旋转引起的文字偏移最多约 0.25 秒后更新，可用 `LabelBoundsUpdateInterval=0` 关闭缓存。
- 不重复写入未变化的文字、可见性和布局；同次更新共享父画布几何，缓存有效的布局库引用。调试日志关闭时不再构造射线仲裁的详细对象字符串。
- Lua 5.4 离线测试共 137 项断言通过；旧 test7 可复现新增失败。实际帧率、崩溃和双客户端显示仍待游戏复测。

## v1.0.4-test7 - 2026-09-07

### 小目标选择与发送保护修复

- 主机自己的 Q 标记使用与 F6 相同的屏幕准星射线；远程玩家仍使用其同步视角，不借用主机镜头。
- 精确射线已命中实体时，附近另一 Actor 的球形探测结果不再覆盖它；保留无精确命中时的球扫兜底、同物体交互组件处理和遮挡距离限制。
- 修正 test6 使用 Lua 包装引用比较世界、Character、PlayerState 和 GameState 的错误。发送保护改为核对 Unreal 地址与完整名称快照，同一对象的新包装不再被误判为换世界。
- 每批发送从当前 Tick 的控制器取得世界，复用该世界读取载体，减少重复全局控制器查询。失败日志分别报告世界变化、角色变化、载体变化、会话变化或超时。
- 保留 38 符号协议、每包 10 批、每发送者单包锁、3 秒冷却和 test6 的输入门控。复杂 Mesh 枚举及一次性描边保持不变。
- 默认开启每次有效射线的目标选择日志，可通过 `TraceSelectionDiagnostics` 关闭；不会增加屏幕调试文字。

### 验证

- 三组新增回归测试在 test6 上失败，test7 修复后通过。当前 Lua 5.4 离线测试共 116 项断言通过，包含新包装对象、同名不同地址、真实编码/发送/解码的 8 槽位回环。
- 实际座机瞄准、UE4SS 原生广播、双客户端显示及帧率仍需安装后进游戏复测。详见 [test7 验证说明](docs/test7验证.md)。

## v1.0.4-test6 - 2026-09-07

### 连按保护与维护性能

- 所有标记按键共用前端门控：最多一个请求等待或执行，冷却期间最多入队一次提示，之后直接忽略。聊天、未命中和本地请求异常不启动完整冷却。
- 冷却使用游戏 Tick 增量，并保留每玩家独立的服务器 3 秒检查；不把 CPU 时间用作冷却时钟。暂停时随游戏时间暂停。
- 数据包由现有游戏线程 Tick 分批发送，每发送者一份状态；移除每包的延迟闭包和重复游戏线程入队。每批核对会话、世界、角色归属、PlayerState 和 GameState，完成、失败或超时释放发送状态。
- 协议 2、38 个符号和默认每批 4 次广播不变；正常 20 Hz 更新下约 0.5 秒完成一个包。所有联机玩家仍应安装相同版本，未验证 test5/test6 混装。
- 文字仍按 0.05 秒更新；普通到期和对象清理改为 0.5 秒周期，维护与文字共用 Tick 累积时间。无工作时不调用 Unreal；回收、到期和失效统一清除标记的附属状态。
- 缓存球体 Mesh 和成功音效，失效或会话重置后刷新。保留 test5 的复杂 Actor Mesh 收集与一次性描边逻辑。
- 增加限频输入计数、发送批次/广播计数、状态表余量和分阶段慢 Tick 日志。

### 验证边界

- Lua 5.4 语法与 71 项离线断言通过，详见 [test6 验证说明](docs/test6验证.md)。
- 旧日志确认为 17 包开始、16 包完成；崩溃报告为 UE4SS 内部访问异常。具体原生根因仍未知，本版是风险修复测试版，不能据离线结果宣称崩溃完全解决或帧率已恢复。
- 真实帧率、连续回收、聊天输入和双客户端显示仍须游戏内复测。

## v1.0.4-test5 - 2026-09-07

### 复杂物体完整 Mesh 描边

- 整体实体描边改用已在游戏内验证可用的 `K2_GetComponentsByClass`，收集根 Actor 与子 Actor 的全部 MeshComponent。
- 继续只处理当前可见且未在游戏中隐藏的 Mesh；碰撞体、交互区域、摄像机、音频与粒子组件不会进入描边。
- 旧组件枚举入口仅在 `K2` 调用失败时作为兼容后备，已知属性映射继续作为最后补充。
- 描边仍只在标记创建时写入一次、结束时检查并恢复一次，没有重新加入持续刷新。
- 默认关闭 diag3 的定向结构日志，减少正式测试中的日志量与额外反射开销。

## v1.0.4-test4-diag3 - 2026-09-07

### 组件枚举诊断修复

- 结构诊断优先使用 UE 蓝图反射入口 `K2_GetComponentsByClass`，分别枚举 Mesh、Primitive 与全部 Actor 组件。
- 每个组件记录其所属集合、可见性、父组件、注册状态与 Custom Depth，用于定位推车货物及风扇叶片的真实组件。
- 移除参数数量错误的 `GetAttachedActors` 后备调用，保留正确的三参数递归查询。
- 本版仍不改变实际描边目标，只修复诊断输出。

## v1.0.4-test4-diag2 - 2026-09-07

### 推车变种诊断覆盖

- 将推车结构诊断关键字由 `MoveableCart` 扩展为 `Cart`，一并覆盖类名中包含该关键字的推车变种。
- 保留 `ElectricFan` 诊断范围以及 diag1 的日志上限，不改变射线或描边行为。

## v1.0.4-test4-diag1 - 2026-09-07

### 平板推车与绝命扇结构诊断

- 仅在标记 `MoveableCart` 或 `ElectricFan` 时，对每个目标 Actor 输出一次有上限的结构日志。
- 补充记录子 Actor、附着 Actor、ChildActorComponent、全部 ActorComponent、组件父级、可见性与 Custom Depth 状态。
- 诊断不扩大射线扫描、不加入逐帧检查，也不改变 v1.0.4-test4 的实际描边选择。

## v1.0.4-test4 - 2026-09-07

### 一次性真实 Mesh 描边测试

- 保留 v1.0.4-test3 的窄射线本地实体解析，不恢复大范围 Actor 球形枚举。
- 取消独立立方体包围盒代理，改为对目标真实可见 Mesh 写入一次 Custom Depth 描边。
- 整体 Actor 标记在创建时一次收集根 Actor 与子 Actor 的可见 Mesh，支持多部件家具一起显示轮廓。
- 10 秒生命周期内不刷新、不轮询 Mesh 描边状态。
- 结束时只检查一次：仍保持本 Mod 标记状态的 Mesh 恢复到标记前数值；已被交互高亮或其他 Mod 改变的 Mesh 完全不动。

## v1.0.4-test3 - 2026-09-07

### 实体本地解析性能测试

- 移除每次实体标记都在命中点周围执行的 `SphereOverlapActors` 大范围 Actor 枚举。
- 各客户端改为沿发送者到命中点的方向，只检查命中点前后一小段射线。
- 先执行精确的简单/复杂对象射线和已知有效通道射线；只有它们都没有解析出实体时，才执行一次 24 单位半径的窄扫掠射线。
- 本地射线直接传递实际命中组件，精确按钮解析不再通过 `FindAllOf` 扫描全场景同类对象。
- 日志新增射线查询数、原始命中数、选中来源与耗时，用于对比联机实测中的卡顿。

## v1.0.4-test2 - 2026-09-07

### 独立包围盒描边测试

- 实体标记不再读取、修改、刷新或归还目标自身任何 Mesh 的 Custom Depth 状态。
- 每台客户端从解析到的根 Actor 出发，汇总其自身及子 Actor 下所有可见 Mesh 的世界包围盒。
- 创建一个本地立方体代理覆盖该包围盒；代理关闭主画面、深度、碰撞和阴影，只写入描边所需的 Custom Depth。
- 代理尝试以保持世界变换的方式绑定根 Actor，由 UE 原生层跟随移动，不使用 Lua 每帧更新。
- 标记结束、目标失效或标记槽回收时直接销毁代理，不影响游戏交互高亮。
- 包围盒计算失败时只保留文字，不再回退为修改目标 Mesh。

## v1.0.4-test1 - 2026-09-07

### 一次性实体描边测试

- 实体描边只在标记创建时写入一次，不再被周期性重新施加。
- 活动期间每 `0.25` 秒只观察描边状态；检测到游戏交互或其他效果修改后，立即永久放弃对应组件的高亮所有权。
- 已被外部修改的组件在标记结束时不会被恢复、清空或再次覆盖。
- 从未发生外部变化且仍保持本 Mod 描边的组件，结束时恢复到标记前状态。
- 本测试版暂不加入复杂物体包围盒代理，以便单独测量一次性描边对帧率的影响。

## v1.0.3 - 2026-09-07

### 持续性能优化

- 没有活动文字、描边、数据包或待处理任务时，玩家 Tick 在执行 Unreal 对象查询前立即返回。
- 本地 Controller/Pawn 完整校验只在活动任务开始、缓存失效或角色变化时执行，不再每帧重复。
- 文字、旧 TextRender 后备和多人/描边维护按各自是否存在活动内容分别执行。
- 实体描边仍每 `0.25` 秒检查一次，但状态正确时不再重复写入 Custom Depth；只有被游戏或其他效果覆盖时才恢复。
- 正式配置默认关闭详细调试日志；超过阈值的活动更新仍会限频记录各阶段耗时和活动对象数量。
- 标记频率、10 秒生命周期、20Hz 活动文字跟踪和多人传输协议保持不变。

## v1.0.2 - 2026-09-07

### 输入保护

- 聊天框或其他文本输入框获得键盘焦点时，按 `Q`、组合键或鼠标中键不再创建标记。
- 输入保护发生在冷却、射线和网络请求之前，被忽略的按键不会消耗冷却时间。
- 默认识别 UMG 的单行、多行文本框，可通过 `config.lua` 的 `IgnoreMarkerWhileTyping` 关闭。
- 焦点检查只在按下标记键时执行，不增加逐帧更新开销。
